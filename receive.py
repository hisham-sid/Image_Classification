...
def handle_pkt(pkt):
    if UDP in pkt and pkt[UDP].dport>9999:
        ...
        contrast=counter[20]*256**3+counter[21]*256**2+counter[22]*256**1+counter[23]
        maxt=counter[24]*256**3+counter[25]*256**2+counter[26]*256**1+counter[27]
        mint=counter[28]*256**3+counter[29]*256**2+counter[30]*256**1+counter[31]
        if (pkt[UDP].sport==10000):
            print("Counter\tLow\tMid\tHigh\tContrast")
            print(num,end=' \t')
            print(low_gray,end='\t ')
            print(mid_gray,end=' \t')
            print(high_gray,end=' \t')
