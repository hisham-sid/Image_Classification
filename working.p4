...
control MyIngress(inout headers hdr,
    ...
    //will need to work on the color types
    action compute_hashes(bit<8> colorR, bit<8> colorG, bit<8> colorB){
       //Get register position
       hash(reg_pos_one, HashAlgorithm.crc16, (bit<32>)0, {colorR,
                                                           colorG,
                                                           colorB},
    ...
    //please check the logic here, i will have to fix some syntaxes
    apply {
        if (hdr.ipv4.isValid()){
            ipv4_lpm.apply();
            if (hdr.tcp.isValid()){
                counter_reg.read(counter, 0)
		compute_hashes(color1,color2,color3);
